      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className={`absolute z-[110] px-3 py-1.5 bg-slate-900 text-white text-[10px] font-bold rounded-lg shadow-xl whitespace-nowrap pointer-events-none ${positionClasses[position]}`}
          >
            {text}
...
export const TerminalLog: React.FC<{ 
  ...
  return (
    <div className="bg-slate-900 rounded-2xl p-4 shadow-xl border border-white/10 relative group">
      ...
      <div className="font-mono text-[9px] leading-relaxed h-[100px] overflow-y-auto custom-scrollbar">
        {logs.length === 0 ? (
...
export const Modal: React.FC<{
  ...
  maxWidth?: string;
  zIndex?: string;
}> = ({ isOpen, onClose, title, children, maxWidth = 'max-w-4xl', zIndex = 'z-[100]' }) => {
  if (!isOpen) return null;

  return (
    <div className={`fixed inset-0 ${zIndex} flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm`}>
      <motion.div 
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className={`bg-white rounded-3xl shadow-2xl w-full ${maxWidth} max-h-[90vh] flex flex-col overflow-hidden`}
      >
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          ...
          <Tooltip text="Zamknij" position="top">
            <button 
              onClick={onClose}
