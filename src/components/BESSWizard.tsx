...
import { Card, TerminalLog, Modal, Tooltip as UITooltip } from './UI';
...
import { BESSConfigManager } from './BESSConfigManager';
...
const BESSWizard: React.FC<BESSWizardProps> = ({ 
  modelingGoal, 
  setModelingGoal, 
  onOpenAuditLogs, 
  onSaveReport, 
  ...
  const [isConfiguringBESS, setIsConfiguringBESS] = useState(false);
  ...
  // BESS Configuration Selection
  const [selectedBessConfigId, setSelectedBessConfigId] = useState<string>('');
  const selectedBessConfig = bessConfigurations.find(c => c.id === selectedBessConfigId) || bessConfigurations[0];
  ...
  // Auto-select config based on company size
  useEffect(() => {
    const defaultConfig = bessConfigurations.find(c => c.companySize === companySize);
    if (defaultConfig) {
      setSelectedBessConfigId(defaultConfig.id);
      addLog(`Automatyczny dobór konfiguracji BESS dla wielkości firmy: ${companySize}`);
    }
  }, [companySize, bessConfigurations]);

  // Update BESS parameters when config changes
  useEffect(() => {
    if (selectedBessConfig) {
      setCapexPerMW(selectedBessConfig.capexPerMW);
      setOpexPerYear(selectedBessConfig.opexPerYear);
      setEfficiency(selectedBessConfig.efficiency / 100);
      addLog(`Zaktualizowano parametry techniczne na podstawie konfiguracji: ${selectedBessConfig.name}`);
    }
  }, [selectedBessConfig]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Flatten the data and filter for numbers
        const values = data.flat()
          .map(v => {
            if (typeof v === 'string') {
              return Number(v.replace(',', '.').replace(/[^\d.-]/g, ''));
            }
            return Number(v);
          })
        ...
        if (values.length > 0) {
          // If we have more than 24 values, we might be importing a full profile or just taking the first 24
          // For this specific section, we need 24 values
          const newValues = [...hourlyValues];
          for (let i = 0; i < Math.min(values.length, 24); i++) {
            newValues[i] = values[i];
          }
          setHourlyValues(newValues);
          addLog(`Zaimportowano ${Math.min(values.length, 24)} wartości z pliku ${file.name}`);
          alert(`Zaimportowano ${Math.min(values.length, 24)} wartości.`);
        } else {
          alert('Nie znaleziono poprawnych wartości liczbowych w pliku.');
        }
      } catch (err) {
        console.error('File processing error:', err);
        alert('Błąd podczas przetwarzania pliku. Upewnij się, że format jest poprawny (CSV lub Excel).');
      }
      // Reset input
    ...
    <div className="space-y-8">
      ...
      <div className="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-accent">
            <Battery size={24} />
          </div>
          <div>
            <h4 className="text-sm font-black text-slate-900 uppercase tracking-tight">Konfiguracja Systemu BESS</h4>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Wybierz predefiniowaną specyfikację techniczną</p>
          </div>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          <select 
            value={selectedBessConfigId}
            onChange={(e) => setSelectedBessConfigId(e.target.value)}
            className="flex-1 md:w-64 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-black text-slate-900 outline-none focus:ring-2 focus:ring-accent/20 transition-all"
          >
            {bessConfigurations.map(config => (
              <option key={config.id} value={config.id}>{config.name} ({config.companySize})</option>
            ))}
          </select>
          <button 
            onClick={() => setIsConfiguringBESS(true)}
            className="p-2.5 bg-slate-50 text-slate-400 hover:text-accent hover:bg-blue-50 rounded-xl border border-slate-200 transition-all"
            title="Zarządzaj konfiguracjami"
          >
            <Settings size={20} />
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Technologia', value: selectedBessConfig?.technology || 'LFP (LiFePO4)', icon: Battery },
          { label: 'Sprawność (RTE)', value: `${selectedBessConfig?.efficiency || 92}%`, icon: Zap },
          { label: 'Głębokość Rozł.', value: `${selectedBessConfig?.depthOfDischarge || 90}% DoD`, icon: TrendingDown },
          { label: 'Żywotność', value: `${selectedBessConfig?.warrantyCycles || 6000} cykli`, icon: Clock },
        ].map((spec, i) => (
          ...
          >
            ...
            <div className="flex justify-center">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-6 py-3 bg-white border border-dashed border-slate-300 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-100 transition-all group"
              >
            ...
            >
              <div className="p-8 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <UITooltip text="Zamknij" position="top">
                    <button 
