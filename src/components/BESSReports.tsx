...
import { Card, KPICard, Modal, Tooltip as UITooltip } from './UI';
...
const BESSReports: React.FC<BESSReportsProps> = ({ reports, setReports, whiteLabel }) => {
  ...
  const [viewMode, setViewMode] = useState<'list' | 'summary' | 'full'>('list');
  ...
  const captureChart = async (elementId: string) => {
    ...
    if (chartElement) {
      console.log(`Found element: ${elementId}, offsetSize: ${chartElement.offsetWidth}x${chartElement.offsetHeight}`);
      // Store original styles to restore later
      const originalWidth = chartElement.style.width;
      const originalHeight = chartElement.style.height;
      const originalMinWidth = chartElement.style.minWidth;
      const originalMinHeight = chartElement.style.minHeight;

      // Force fixed dimensions for capture to ensure Recharts renders correctly
      chartElement.style.width = '1200px';
      chartElement.style.height = '600px';
      chartElement.style.minWidth = '1200px';
      chartElement.style.minHeight = '600px';

      // Small delay to ensure Recharts has finished rendering with new dimensions
      await new Promise(resolve => setTimeout(resolve, 500));
      try {
        console.log(`Starting html2canvas for: ${elementId}`);
        ...
        console.log(`Successfully captured chart: ${elementId}, canvas size: ${canvas.width}x${canvas.height}`);
        // Restore original styles
        chartElement.style.width = originalWidth;
        chartElement.style.height = originalHeight;
        chartElement.style.minWidth = originalMinWidth;
        chartElement.style.minHeight = originalMinHeight;
  ...
  const handleConfirmDownloadPdf = async () => {
    ...
    const originalFullReportModalOpen = isFullReportModalOpen;
    
    try {
      // Ensure we are in full report mode and modal is open to capture the chart
      if (viewMode !== 'full' || !isFullReportModalOpen) {
        setViewMode('full');
        setIsFullReportModalOpen(true);
        // Wait for render and for charts to stabilize
      ...
      const pdfDoc = (
        <ReportPDF 
          ...
          branding={whiteLabel} 
          chartImage={capturedChart1 || undefined} 
          chartImage2={capturedChart2 || undefined}
          chartImage3={capturedChart3 || undefined}
          chartImage4={capturedChart4 || undefined}
        />
      );
      ...
      console.log('Starting PDF rendering with data:', previewReport);
      ...
      try {
        ...
        console.error('Error during pdf().toBlob():', pdfError);
        throw pdfError;
      }
      
      // Restore original states if they were changed
      if (originalViewMode !== 'full') setViewMode(originalViewMode);
      if (!originalFullReportModalOpen) setIsFullReportModalOpen(false);
      ...
      // Restore original states on error too
      if (originalViewMode !== 'full') setViewMode(originalViewMode);
      if (!originalFullReportModalOpen) setIsFullReportModalOpen(false);
  ...
  return (
    <div className="space-y-8">
      ...
      {viewMode === 'list' ? (
            ...
            <tbody className="divide-y divide-slate-50">
              {paginatedReports.map((report) => (
                <tr key={report.id} className={`group hover:bg-slate-50/50 transition-colors ${selectedReportIds.includes(report.id) ? 'bg-blue-50/30' : ''}`}>
                  ...
                  <td className="py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <UITooltip text="Podgląd" position="top">
                        <button 
                          onClick={() => {
                            setPreviewReport(report);
                            setViewMode('summary');
                      ...
                      <UITooltip text="Pobierz PDF" position="top">
                        <button 
                          onClick={() => {
                            setPreviewReport(report);
                      ...
                      <UITooltip text="Więcej opcji" position="top">
        ...
        <div className="mt-8 pt-8 border-t border-slate-100 flex items-center justify-between">
          <p className="text-xs text-slate-400 font-medium">
            Wyświetlono {paginatedReports.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} - {Math.min(currentPage * itemsPerPage, filteredReports.length)} z {filteredReports.length} raportów
          </p>
          <div className="flex gap-2">
            <button 
      ...
      ) : viewMode === 'summary' && previewReport ? (
        <motion.div
        ...
        >
          <div className="flex items-center justify-between">
            ...
            >
              <ChevronUp className="-rotate-90" size={16} /> Powrót do listy
            </button>
            ...
            <div className="flex items-center gap-3">
              <button 
                onClick={async () => {
                  try {
                    ...
                    const SimpleDoc = () => (
                      <Document>
                        <Page size="A4" style={{ padding: 40 }}>
                          <Text style={{ fontSize: 24, marginBottom: 20 }}>Test PDF - Tylko Tekst</Text>
                          <Text style={{ fontSize: 12 }}>To jest testowy plik PDF wygenerowany w celu sprawdzenia działania biblioteki @react-pdf/renderer.</Text>
                          <Text style={{ fontSize: 12, marginTop: 10 }}>Data: {new Date().toLocaleString()}</Text>
                        </Page>
                      </Document>
                    );
                    ...
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = 'Test_PDF_Tekst.pdf';
                    link.click();
                    URL.revokeObjectURL(url);
                    console.log('Simple Test PDF generated successfully.');
                  } catch (err) {
                    console.error('Simple Test PDF failed:', err);
                  }
                }}
              ...
              <button 
                onClick={async () => {
                  try {
                    console.log('Generating Drawing Test PDF...');
                    const DrawingDoc = () => (
                      <Document>
                        <Page size="A4" style={{ padding: 40 }}>
                          <Text style={{ fontSize: 24, marginBottom: 20 }}>Test PDF - Rysunki</Text>
                          <View style={{ width: 100, height: 100, backgroundColor: '#3B82F6', borderRadius: 10, marginBottom: 20 }} />
                          <View style={{ width: 200, height: 20, backgroundColor: '#10B981', marginBottom: 20 }} />
                          <View style={{ borderBottomWidth: 2, borderBottomColor: '#E2E8F0', marginBottom: 20 }} />
                          <Text style={{ fontSize: 12 }}>To jest testowy plik PDF z prostymi rysunkami (View components).</Text>
                        </Page>
                      </Document>
                    );
                    const blob = await pdf(<DrawingDoc />).toBlob();
                    const url = URL.createObjectURL(blob);
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = 'Test_PDF_Rysunki.pdf';
                    link.click();
                    URL.revokeObjectURL(url);
                    console.log('Drawing Test PDF generated successfully.');
                  } catch (err) {
                    console.error('Drawing Test PDF failed:', err);
                  }
                }}
                className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all"
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg"
              >
                <FileText size={14} /> Pełny Raport
              </button>
            </div>
            </div>
          </div>
            ...
            <div className="space-y-8 p-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-black text-slate-900 tracking-tight">{previewReport.name}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="flex items-center gap-2 text-slate-400 mb-3">
                    <TrendingUp size={18} />
                    <span className="text-[10px] font-black uppercase tracking-widest">Oszczędności</span>
                  </div>
                  <p className="text-3xl font-black text-emerald-600">{formatCurrency(previewReport.savings)}</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Szacowane rocznie</p>
                </div>
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="flex items-center gap-2 text-slate-400 mb-3">
                    <Zap size={18} />
                  ...
                  <p className="text-3xl font-black text-slate-900">{previewReport.roi}%</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Zwrot z inwestycji</p>
                </div>
                <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="flex items-center gap-2 text-slate-400 mb-3">
                    <Calendar size={18} />
                    <span className="text-[10px] font-black uppercase tracking-widest">Zwrot</span>
                  </div>
                  <p className="text-3xl font-black text-slate-900">{previewReport.payback} lat</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Okres amortyzacji</p>
                </div>
              </div>

              <div className="bg-blue-50/50 rounded-3xl p-8 border border-blue-100/50">
                <h4 className="text-xs font-black text-accent uppercase tracking-widest mb-6">Konfiguracja Systemu</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pojemność Magazynu</p>
                    <p className="text-lg font-black text-slate-900">{previewReport.capacity} kWh</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Moc Systemu</p>
                    <p className="text-lg font-black text-slate-900">{previewReport.power} kW</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Typ Klienta</p>
                    <p className="text-lg font-black text-slate-900 capitalize">{previewReport.customerType}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Autor</p>
                    <p className="text-lg font-black text-slate-900">{previewReport.author}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <button 
              ...
              className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors font-bold uppercase tracking-widest text-[10px]"
            >
              <ChevronUp className="-rotate-90" size={16} /> Powrót do podsumowania
            </button>
            <button 
              onClick={() => handleDownloadPdf(previewReport)}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-100 transition-all"
          ...
          <div className="bg-white p-8 md:p-12 rounded-[32px] shadow-xl border border-slate-100 overflow-hidden">
            <div className="text-center py-12">
              <div className="w-20 h-20 bg-blue-50 text-blue-500 rounded-3xl flex items-center justify-center mx-auto mb-6">
                <FileText size={40} />
              </div>
              <h3 className="text-2xl font-black text-slate-900 mb-2">Pełny Raport Techniczny</h3>
              <p className="text-slate-500 max-w-md mx-auto mb-8">Szczegółowa analiza została otwarta w dedykowanym oknie podglądu dla lepszej czytelności i możliwości wydruku.</p>
              <button 
                onClick={() => setIsFullReportModalOpen(true)}
                className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-800 transition-all shadow-lg"
              >
                Otwórz ponownie okno raportu
              </button>
            </div>
          </div>
        </motion.div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={0} aspect={2}>
      ...
      >
        {previewReport && (
          <div className="space-y-12 pb-12">
              ...
              <section className="mb-16 pt-16 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-12">
                  <div id="report-chart-container" className="h-[350px] bg-slate-50 rounded-3xl p-8 border border-slate-100">
                    ...
                    <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={0}>
                      <BarChart data={[
                        ...
                        <YAxis 
                          ...
                          tickLine={{ stroke: '#94a3b8' }} 
                          tick={{ fill: '#475569', fontSize: 10, fontWeight: 700 }}
                          label={{ value: 'PLN', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 10, fontWeight: 700 }}
                        />
                        <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                        <Bar dataKey="value" radius={[6, 6, 0, 0]} stroke="#1e293b" strokeWidth={1}>
                          {[{ color: '#94a3b8' }, { color: '#3b82f6' }, { color: '#10b981' }].map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={entry.color} />
                          ))}
                          <LabelList dataKey="value" position="top" fill="#1e293b" fontSize={10} fontWeight={700} formatter={(v: number) => `${Math.round(v / 1000)}k`} />
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  <div id="report-savings-structure" className="h-[300px] bg-slate-50 rounded-3xl p-8 border border-slate-100">
                    ...
                    <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={0}>
                      <BarChart data={[
                        ...
                        />
                        <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                        <Bar dataKey="v" fill="#3b82f6" radius={[4, 4, 0, 0]} stroke="#1e293b" strokeWidth={1}>
                          <LabelList dataKey="v" position="top" fill="#1e293b" fontSize={8} fontWeight={700} formatter={(v: number) => Math.round(v).toLocaleString()} />
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                ...
                <div id="report-cumulative-savings" className="h-[350px] bg-slate-50 rounded-3xl p-8 border border-slate-100 mb-8">
                  <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={0}>
                    <BarChart data={[
                      ...
                      />
                      <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                      <Bar dataKey="value" fill="#3b82f6" radius={[4, 4, 0, 0]} stroke="#1e293b" strokeWidth={1}>
                        <LabelList dataKey="value" position="top" fill="#1e293b" fontSize={9} fontWeight={700} formatter={(v: number) => `${Math.round(v / 1000)}k`} />
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              ...
              <section className="mb-16 pt-16 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {[
                        { label: 'Technologia Ogniw', value: previewReport.bessConfig?.technology || 'LiFePO4 (LFP)', detail: 'Najwyższy poziom bezpieczeństwa pożarowego i trwałości.' },
                        { label: 'Inwerter PCS', value: previewReport.bessConfig?.inverterType || 'Bi-directional Hybrid', detail: 'Możliwość pracy wyspowej (Grid-forming) i czarny start.' },
                        { label: 'System BMS', value: previewReport.bessConfig?.bmsSystem || '3-Level Active Balancing', detail: 'Precyzyjne monitorowanie stanu SOC/SOH każdego ogniwa.' },
                        { label: 'Chłodzenie', value: previewReport.bessConfig?.coolingType || 'Liquid Cooling (HVAC)', detail: 'Utrzymanie optymalnej temp. pracy dla wydłużenia życia ogniw.' },
                        { label: 'Obudowa', value: previewReport.bessConfig?.housing || 'IP55 Outdoor Container', detail: 'Odporność na trudne warunki atmosferyczne (C5).' },
                      ].map((item, i) => (
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-black text-amber-900">{previewReport.bessConfig ? `${previewReport.bessConfig.warrantyYears} LAT / ${previewReport.bessConfig.warrantyCycles} CYKLI` : '10 LAT / 6000 CYKLI'}</p>
                          </div>
                        </div>
                        <p className="text-xs text-amber-800 leading-relaxed font-medium">
                          Gwarantujemy zachowanie minimum {previewReport.bessConfig?.depthOfDischarge || 70}% pojemności znamionowej po {previewReport.bessConfig?.warrantyYears || 10} latach lub {previewReport.bessConfig?.warrantyCycles || 6000} pełnych cyklach ładowania/rozładowania.
                        </p>
                      </div>
                      <div className="space-y-4">
                        <div className="flex items-center gap-3">
                      ...
                      <ResponsiveContainer width="100%" height="100%" minWidth={100} minHeight={100} debounce={0}>
                        <BarChart data={[
                          ...
                          <YAxis 
                            ...
                            tickLine={{ stroke: '#94a3b8' }} 
                            tick={{ fill: '#475569', fontSize: 10, fontWeight: 700 }}
                            label={{ value: 'Lata', angle: -90, position: 'insideLeft', fill: '#94a3b8', fontSize: 10, fontWeight: 700 }}
                          />
                          <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }} />
                          <Bar dataKey="value" radius={[4, 4, 0, 0]} stroke="#1e293b" strokeWidth={1}>
                            {[
                              { color: '#3b82f6' },
                              { color: '#94a3b8' },
                              { color: '#94a3b8' },
                            ...
                            <LabelList dataKey="value" position="top" fill="#1e293b" fontSize={10} fontWeight={700} formatter={(v: number) => `${v.toFixed(1)} lat`} />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
            ...
            {/* Action Buttons */}
            <div className="flex justify-center gap-4 py-8 bg-slate-50 border-t border-slate-100">
              <button 
                onClick={() => handleDownloadPdf(previewReport)}
                disabled={isGeneratingPdf}
              ...
              <button 
                onClick={() => {
                  setIsFullReportModalOpen(false);
                  if (viewMode === 'full') setViewMode('summary');
                }}
                className="px-10 py-4 bg-slate-200 text-slate-700 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-slate-300 transition-all"
              >
                Zamknij Podgląd
              </button>
            </div>
