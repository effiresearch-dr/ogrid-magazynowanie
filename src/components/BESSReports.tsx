...
const BESSReports: React.FC<BESSReportsProps> = ({ reports, setReports, whiteLabel }) => {
  ...
  const handleConfirmDownloadPdf = async () => {
    ...
    try {
      ...
      setGenerationStep('Budowanie dokumentu PDF (7 stron)...');
            ...
            <div id="full-report-content" className="bg-white p-12 rounded-3xl shadow-sm border border-slate-100 font-sans text-slate-900">
              ...
              <section className="mb-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                  <div className="space-y-6">
                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2">Porównanie Kosztów Energii</h4>
                    <div className="overflow-hidden rounded-2xl border border-slate-100">
                      <table className="w-full text-left text-xs">
                        <thead className="bg-slate-50">
                          <tr>
                            <th className="px-4 py-3 font-black text-slate-400 uppercase tracking-widest">Okres</th>
                            <th className="px-4 py-3 font-black text-slate-400 uppercase tracking-widest">Bez BESS</th>
                            <th className="px-4 py-3 font-black text-slate-400 uppercase tracking-widest">Z BESS</th>
                            <th className="px-4 py-3 font-black text-emerald-600 uppercase tracking-widest">Oszczędność</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {[
                            ...
                            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-4 py-3 text-slate-500">{formatCurrency(row.without)}</td>
                              <td className="px-4 py-3 text-slate-500">{formatCurrency(row.with)}</td>
                              <td className="px-4 py-3 font-black text-emerald-600">-{formatCurrency(row.diff)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
