...
import { Card, Tooltip as UITooltip } from './UI';
...
interface ConfiguratorsProps {
  initialTab?: 'rules' | 'prices' | 'white-label' | 'report-builder' | 'bess';
  whiteLabel: {
    companyName: string;
    logoUrl: string;
    primaryColor: string;
...
const Configurators: React.FC<ConfiguratorsProps> = ({ 
  initialTab, 
  whiteLabel, 
  onUpdateWhiteLabel, 
  onTabChange, 
  ...
  const [activeTab, setActiveTab] = useState<'rules' | 'prices' | 'white-label' | 'report-builder' | 'bess'>(initialTab || 'prices');

  const handleTabChange = (tab: 'rules' | 'prices' | 'white-label' | 'report-builder' | 'bess') => {
    setActiveTab(tab);
    if (onTabChange) {
      const subTabId = tab === 'rules' ? 'config-rules' : 
                       tab === 'white-label' ? 'config-white-label' : 
                       tab === 'report-builder' ? 'config-report-builder' : 
                       tab === 'bess' ? 'config-bess' :
                       'config-prices';
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (values.length > 0) {
          ...
          setElectricityPrices(electricityPrices.map(ep => ep.id === uploadTargetId ? { ...ep, values } : ep));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-2xl w-fit">
        ...
        <button 
          onClick={() => handleTabChange('bess')}
          className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'bess' ? 'bg-white text-accent shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
        >
          Specyfikacja BESS
        </button>
      </div>
                    ...
                    >
                      ...
                      <div className="col-span-4 flex flex-col gap-1">
                        <div className="flex gap-2">
                          ...
                          />
                          <UITooltip text="Wklej ze schowka">
                            <button 
                  ...
                  <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2 p-1 bg-slate-200/50 rounded-xl w-fit">
                          <button 
                            onClick={() => setForecastSettings({...forecastSettings, energyPriceGrowthType: 'linear'})}
                            className={`px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all ${forecastSettings.energyPriceGrowthType === 'linear' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                          >
                            Liniowy (%)
                          </button>
                          <button 
                            onClick={() => setForecastSettings({...forecastSettings, energyPriceGrowthType: 'nonlinear'})}
                            className={`px-4 py-1.5 rounded-lg text-[10px] font-bold transition-all ${forecastSettings.energyPriceGrowthType === 'nonlinear' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                          >
                            Nieliniowy (Indywidualny)
                          </button>
                        </div>
                      ...
                      {forecastSettings.energyPriceGrowthType === 'linear' ? (
                        ...
                        <div className="space-y-3">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Wzrost w kolejnych latach (%)</label>
                          <div className="grid grid-cols-5 gap-2">
                            {forecastSettings.energyPriceNonlinearRates.map((rate, i) => (
                              <div key={i}>
                                <label className="text-[8px] font-bold text-slate-400 block mb-1 text-center">Rok {i+1}</label>
                                <input 
                                  type="number"
                                  value={rate}
                                  onChange={(e) => {
                                    const newRates = [...forecastSettings.energyPriceNonlinearRates];
                                    newRates[i] = Number(e.target.value);
                                    setForecastSettings({...forecastSettings, energyPriceNonlinearRates: newRates});
                                  }}
                                  className="w-full bg-white border border-slate-200 rounded-lg px-2 py-1 text-[10px] font-bold text-center outline-none focus:ring-2 focus:ring-emerald-500/20"
                                />
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="h-[250px]">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={Array.from({length: 10}).map((_, i) => {
                          ...
                          let energyFactor = 1;
                          if (forecastSettings.energyPriceGrowthType === 'linear') {
                            energyFactor = Math.pow(1 + forecastSettings.energyPriceGrowthRate/100, i);
                          } else {
                            for (let j = 0; j <= i; j++) {
                              energyFactor *= (1 + forecastSettings.energyPriceNonlinearRates[j]/100);
                            }
                          }
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-3 bg-white/5 rounded-2xl">
                        ...
                        <div className="text-xl font-black text-emerald-400">
                          +{Math.round((forecastSettings.energyPriceGrowthType === 'linear'
                            ? Math.pow(1 + forecastSettings.energyPriceGrowthRate/100, 10)
                            : forecastSettings.energyPriceNonlinearRates.reduce((acc, r) => acc * (1 + r/100), 1)
                          ) * 100 - 100)}%
                        </div>
                      </div>
                    </div>
                ...
                {tariffs.map((tariff) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <UITooltip text="Usuń taryfę" position="top">
                        <button 
                        ...
                        <div className="space-y-2">
                          {tariff.zones.map((zone) => (
                            <div key={zone.id} className="grid grid-cols-12 gap-2 items-center bg-white p-2 rounded-xl border border-slate-200">
                              ...
                              <div className="col-span-1 text-right">
                                <UITooltip text="Usuń strefę" position="top">
                                  <button 
                                    onClick={() => {
                ...
                {prices.map((price) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="col-span-1 text-right">
                        <UITooltip text="Usuń wpis" position="top">
                          <button 
