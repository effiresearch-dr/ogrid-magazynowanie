  ...
  const menuItems = [
    ...
    { 
      ...
      subItems: [
        ...
        { id: 'config-bess', label: 'Specyfikacja BESS' },
        { id: 'config-white-label', label: 'White-Label' },
        { id: 'config-report-builder', label: 'Kreator Raportów' },
      ]
    },
    ...
    { 
      ...
      subItems: [
        { id: 'settings-users', label: 'Użytkownicy' },
        { id: 'settings-audit', label: 'Logi Audytowe' },
      ]
    },
  ];
            ...
            <button
              onClick={() => {
                if (item.hasSubmenu) {
                  ...
                  } else if (item.id === 'settings' && !activeSubTab.startsWith('settings-')) {
                    setActiveSubTab('settings-users');
                  }
                } else {
                  setActiveTab(item.id);
                  setActiveSubTab('');
        ...
        <div className="flex items-center gap-3 px-2 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-slate-500 uppercase font-bold">
              {user?.role === 'admin_app' ? 'Admin App' : 
               user?.role === 'admin_firmy' ? 'Admin Firmy' : 'Użytkownik'}
            </p>
          </div>
          <UITooltip text="Wyloguj się" position="right">
            <button 
              onClick={() => signOut()}
