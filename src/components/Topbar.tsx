...
const Topbar: React.FC<TopbarProps> = ({ title, activeTab, activeSubTab, modelingGoal, onNavigate }) => {
  const { user } = useAuth();
  const getSubTabLabel = (id: string) => {
    switch (id) {
      ...
      case 'settings-users': return 'Użytkownicy';
      case 'settings-audit': return 'Logi Audytowe';
      case 'config-prices': return 'Ceny i stawki';
      case 'config-rules': return 'Reguły wnioskowania';
      case 'config-bess': return 'Specyfikacja BESS';
      case 'config-white-label': return 'White-Label';
      case 'config-report-builder': return 'Kreator Raportów';
      default: return '';
    }
  };
  ...
  return (
    <header className="h-14 bg-white border-b border-slate-100 flex items-center justify-between px-8 sticky top-0 z-40">
      <div className="flex items-center gap-4">
        <nav className="flex items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest gap-1.5">
          <button 
          ...
          <button 
            onClick={() => {
              if (activeTab === 'bess') onNavigate('bess', 'bess-desc');
              else if (activeTab === 'configurators') onNavigate('configurators', 'config-prices');
              else if (activeTab === 'settings') onNavigate('settings', 'settings-users');
              else onNavigate(activeTab);
            }}
            className={`transition-colors hover:text-accent ${!activeSubTab ? 'text-slate-900' : 'hover:text-accent'}`}
          >
            {title}
          </button>
          {activeSubTab && (
      ...
      <div className="flex items-center gap-6">
        ...
        <UITooltip text="Powiadomienia" position="bottom">
        ...
        <UITooltip text="Profil Użytkownika" position="bottom">
